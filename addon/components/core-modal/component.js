...
if (DEVELOPMENT) {
  devAssets.didInsertElement = function() {
    ...
    if (!$(headerId).is('header')) {
      ...
      throw new Error('{{core-modal}}: You must specify a modal header or supply an `ariaHeader` string, ya dongus');
    }
  };
}
 ...
 * **With Basic Header:**
 * ```handlebars
 * {{#core-modal open=someService.modalActive Header='This is the modal title' closeModal=(action "closeModal" target=someService)}}
 ...
 * ```handlebars
 * {{#core-modal open=someService.modalActive closeModal=(action "closeModal" target=someService) as |components|}}
 *   {{! Modal Header Subcomponent}}
 *   {{#components.header}}
 *     <h3>{{core-svg svgId="sparkles"}} It Just Works™</h3>
 *   {{/components.header}}
 ...
 *
 * ```handlebars
 * {{#core-modal open=someService.modalActive ariaHeader='This is the puppy modal' closeModal=(action "closeModal" target=someService)}}
  ...
  layout: hbs`
    ...
    {{#if (or _isVisible (not removeFromDomOnClose))}}
      ...
      <div class="core-modal-wrapper {{size}} {{if animateFrom (concat 'animate-' animateFrom)}}"
        ...
        {{! ----------------------------------------------------------------- }}
        {{yield (hash
          header=(component 'core-modal/header'
            closeModal=closeModal
            aria-labelledby=(concat 'aria-labelledby-' elementId)
            hideX=hideX
            tagClose=tagClose)
