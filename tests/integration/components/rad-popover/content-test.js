...
module('Integration | Component | rad popover/content', function(hooks) {
  ...
  test('it renders', async function(assert) {
    ...
    assert.equal(
      ...
      'Render test content',
      'The popover content subcomponent should display its supplied content',
    )
  })
  ...
  test('aria-describedby is translated to component ID attr', async function(assert) {
    ...
    assert.equal(
      ...
      this.get('aria-describedby'),
      'The ID of the content subcomponent element should match the value of the aria-describedby property passed into it.',
    )
  })
  ...
  test('Position attribute applies the correct classes', async function(assert) {
    ...
    assert.ok(
      find('[data-test="position-test-content"]').classList.contains('top'),
      'The position attribute should apply the corresponding positional CSS class',
    )

    this.set('position', 'bottom-right')

    assert.ok(
      find('[data-test="position-test-content"]').classList.contains(
        'bottom-right',
      ),
      'The position attribute should be correctly updated with the appropriate positional CSS class',
    )
  })
  ...
  test('Size attribute applies the correct classes', async function(assert) {
    ...
    assert.ok(
      find('[data-test="size-test-content"]').classList.contains('large'),
      'The size attribute should apply the corresponding sizing CSS class',
    )

    this.set('size', 'small')

    assert.ok(
      find('[data-test="size-test-content"]').classList.contains('small'),
      'The size attribute should be correctly updated with the appropriate sizing CSS class',
    )
  })
})
