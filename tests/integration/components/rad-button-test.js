...
module('Integration | Component | rad button', function(hooks) {
  ...
  test('it renders', async function(assert) {
    ...
    assert.equal(find('*').textContent.trim(), 'Default button', 'component renders block form correctly');
    assert.equal(findAll('button').length, 1, 'component is a button element');
    assert.ok(find('button').classList.contains('btn'), 'component always bind class btn');
    assert.ok(find('button').classList.contains('rad-button'), 'component renders addon class name');
  });

  test('it renders brand classes', async function(assert) {
    await render(hbs`{{#rad-button brand="success"}}Default button{{/rad-button}}`);
    assert.ok(find('button').classList.contains('btn-success'), 'the component binds branding class');
  });

  test('it renders outline classes', async function(assert) {
    await render(hbs`{{#rad-button brand="success" outline=true}}Default button{{/rad-button}}`);
    assert.ok(find('button').classList.contains('btn-outline-success'), 'the component binds outline class');
    assert.notOk(find('button').classList.contains('btn-success'), 'the component binds ONLY outline class');
  });

  test('it uses prop link to output link class', async function(assert) {
    // Not configured default link=false
    await render(hbs`{{#rad-button}}Default button{{/rad-button}}`);
    assert.notOk(find('button').classList.contains('btn-link'), 'component does not render btn-link by default');

    // link = false
    await render(hbs`{{#rad-button link=false}}Default button{{/rad-button}}`);
    assert.notOk(find('button').classList.contains('btn-link'), 'component does not render btn-link when link=false');

    // explicit link = true
    await render(hbs`{{#rad-button link=true}}Default button{{/rad-button}}`);
    assert.ok(find('button').classList.contains('btn-link'), 'component renders btn-link class when link=true');
  });
  ...
  test('it binds passed attributes', async function(assert) {
    ...
    assert.equal(find('button').getAttribute('aria-describedby'), 'describes',
