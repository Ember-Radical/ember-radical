...
module('Integration | Component | rad drawer', function(hooks) {
  ...
  test('it renders (simple invocation)', async function(assert) {
    this.setProperties({
      contentText:
        'Imagine five paragraphs about puppies instead of this one sentence.',
    ...
    assert.ok(
      findAll('[data-test="basic-render-test-target"]').length,
      'component renders target div via simple invocation',
    )
    assert.ok(
      findAll('[data-test="basic-render-test-content"]').length,
      'component renders content div via simple invocation',
    )

    // Test data-test existence and passage of text content
    assert.equal(
      find('[data-test="basic-render-test-target-yield"]').textContent.trim(),
      this.get('targetText'),
      'component renders the target text supplied via Target attr into the target subcomponent',
    )
    assert.equal(
      find('[data-test="basic-render-test-content"]').textContent.trim(),
      this.get('contentText'),
      'component renders the target text supplied via Content attr into the content subcomponent',
    )

    // Test aria-controls bound to id
    const targetControls = find(
      '[data-test="basic-render-test-target"]',
    ).getAttribute('aria-controls')
    assert.ok(targetControls, 'target subcomponent has aria-controls value')
    assert.equal(
      findAll(`#${targetControls}`).length,
      1,
      'target subcomponent aria-controls matches one id in DOM',
    )
  })
  ...
  test('it yields target and content contextual subcomponents', async function(assert) {
    ...
    assert.ok(
      findAll('[data-test="subcomponent-test-target"]').length,
      'the component renders a target contextual subcomponent',
    )
    assert.ok(
      findAll('[data-test="subcomponent-test-content"]').length,
      'the component renders a content contextual subcomponent',
    )

    // Validate subcomponents content
    assert.equal(
      find('[data-test="subcomponent-test-target-yield"]').textContent.trim(),
      'Test Target',
      'the target subcomponent renders its content correctly',
    )
    assert.equal(
      find('[data-test="subcomponent-test-content"]').textContent.trim(),
      this.get('testContent'),
      'the content subcomponent renders its content correctly',
    )

    // Validate aria-expanded on target subcomponent
    assert.equal(
      find('[data-test="subcomponent-test-target"]').getAttribute('aria-expanded'),
      'false',
      'target subcomponent is not expanded by default',
    )

    // Validate aria-hidden on content subcomponent
    assert.equal(
      find('[data-test="subcomponent-test-content"]').getAttribute('aria-hidden'),
      'true',
      'content subcomponent is hidden by default',
    )
  })
  ...
  test('it uses the correct style for the target subcomponent', async function(assert) {
    ...
    assert.ok(
      find('[data-test="buttonStyle-test-target"]').classList.contains('btn-link'),
      'target renders as a link by default',
    )
    ...
    assert.notOk(
      find('[data-test="buttonStyle-test-target"]').classList.contains('btn-link'),
      'target renders as a button when buttonStyle is set to true',
    )
  })
  ...
  test('it expands and collapses the drawer when the target is clicked', async function(assert) {
    ...
    assert.equal(
      find('[data-test="open-test-target"]').getAttribute('aria-expanded'),
      'false',
      'target subcomponent is not aria-expanded by default',
    )
    assert.equal(
      find('[data-test="open-test-content"]').getAttribute('aria-hidden'),
      'true',
      'content subcomponent is hidden by default',
    )
    ...
    assert.equal(
      find('[data-test="open-test-target"]').getAttribute('aria-expanded'),
      'true',
      'target subcomponent is aria-expanded after click',
    )
    assert.equal(
      find('[data-test="open-test-content"]').getAttribute('aria-hidden'),
      'false',
      'content subcomponent is no longer aria-hidden after click',
    )
    ...
    assert.equal(
      find('[data-test="open-test-target"]').getAttribute('aria-expanded'),
      'false',
      'target subcomponent is not aria-expanded after collapse',
    )
    assert.equal(
      find('[data-test="open-test-content"]').getAttribute('aria-hidden'),
      'true',
      'content subcomponent is hidden after collapse',
    )
  })
  ...
  test('it expands when external state is manipulated', async function(assert) {
    ...
    // Validate that the content container is collapsed
    assert.equal(
      find('[data-test="external-state-test-target"]').getAttribute(
        'aria-expanded',
      ),
      'false',
      'target subcomponent is not aria-expanded by default',
    ...
    assert.equal(
      find('[data-test="external-state-test-content"]').getAttribute('aria-hidden'),
      'true',
      'content subcomponent is hidden by default',
    )
    ...
    assert.equal(
      ...
      'true',
      'target subcomponent is aria-expanded after external state changes',
    )
    assert.equal(
      find('[data-test="external-state-test-content"]').getAttribute('aria-hidden'),
      'false',
      'content subcomponent is no longer aria-hidden after external state changes',
    )
    ...
    assert.equal(
      ...
      'false',
      'target subcomponent is not aria-expanded after external state changes back to false',
    )
    assert.equal(
      find('[data-test="external-state-test-content"]').getAttribute('aria-hidden'),
      'true',
      'content subcomponent is hidden after external state changes back to false',
    )
  })
  ...
  test('the drawer icon is disabled when icon is set to false', async function(assert) {
    ...
    assert.notOk(
      findAll('[data-test="no-icon-test-icon"]').length,
  ...
  test('the custom drawer icon is correctly referenced', async function(assert) {
    ...
    assert.ok(
      find('[data-test="custom-icon-test-target-icon"] use')
        .getAttribute('href')
        .includes('#steve-brule'),
