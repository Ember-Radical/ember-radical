...
module('Integration | Component | rad modal', function(hooks) {
  ...
  test('it renders all the bits', async function(assert) {
    ...
    assert.ok(find('*').textContent.includes('template block text'),
      'Component renders block form correctly');
    assert.equal(find('.rad-modal-wrapper').getAttribute('role'), 'dialog',
      'Component renders role dialog');
    assert.ok(findAll('[data-test="rad-modal-close-button"]').length,
  ...
  test('it renders a passed size', async function(assert) {
    ...
    assert.equal(findAll('.rad-modal-wrapper.medium').length, 1,
  ...
  test('it binds close button svgId to passed closeIcon', async function(assert) {
    ...
    assert.ok(find('[data-test="rad-modal-close-button"] svg').classList.contains('arrow-down'),
  ...
  test('it binds aria-labelledby to passed ariaHeader', async function(assert) {
    ...
    assert.equal(findAll('[data-test="rad-modal-close-button"]').length, 1,
      'should render a close x inside of the header placeholder');

    // Hide the close button
    this.set('closeButton', false);
    assert.equal(ariaLabel, boundId, 'aria-labelledby matches bound id for aria header');
    assert.equal(findAll('[data-test="rad-modal-close-button"]').length, 0,
  ...
  test('it toggles aria-hidden attrs appropriately to handle show and hide', async function(assert) {
    ...
    return settled().then(() => {
      // Hidden by default
      // ---------------------------------------------------------------------------
      assert.equal(find('.rad-modal-background').getAttribute('aria-hidden'), 'true',
        'background aria-hidden true when open is falsey');
      assert.equal(getComputedStyle(find('.rad-modal-background')).display, 'none',
        'background display none when open is falsey');
      assert.equal(getComputedStyle(find('.rad-modal-background')).visibility, 'hidden',
        'background hidden when open is falsey');
      assert.equal(getComputedStyle(find('.rad-modal-background'))['pointer-events'], 'none',
        'clicks disabled when open is falsey');

      assert.equal(find('.rad-modal-wrapper').getAttribute('aria-hidden'), 'true',
        'wrapper aria-hidden true when open is falsey');
      assert.equal(getComputedStyle(find('.rad-modal-wrapper')).display, 'none',
        'wrapper display none when open is falsey');
      assert.equal(getComputedStyle(find('.rad-modal-wrapper')).visibility, 'hidden',
        'wrapper hidden when open is falsey');
      assert.equal(getComputedStyle(find('.rad-modal-wrapper'))['pointer-events'], 'none',
        'clicks disabled when open is falsey');
      ...
      return settled().then(() => {
        // Show the modal
        // --------------------------------------------s-------------------------------
        assert.equal(find('.rad-modal-background').getAttribute('aria-hidden'),
          'false', 'background aria-hidden false when open is truthy');
        assert.equal(getComputedStyle(find('.rad-modal-background')).display, 'block',
          'background display block when open is truthy');
        assert.equal(getComputedStyle(find('.rad-modal-background')).visibility, 'visible',
          'background not hidden when open is truthy');
        assert.equal(getComputedStyle(find('.rad-modal-background'))['pointer-events'], 'auto',
          'clicks not disabled when open is truthy');

        assert.equal(find('.rad-modal-wrapper').getAttribute('aria-hidden'), 'false',
          'wrapper aria-hidden false when open is truthy');
        assert.equal(getComputedStyle(find('.rad-modal-wrapper')).display, 'block',
          'wrapper display block when open is truthy');
        assert.equal(getComputedStyle(find('.rad-modal-wrapper')).visibility, 'visible',
          'wrapper not hidden when open is truthy');
        assert.equal(getComputedStyle(find('.rad-modal-wrapper'))['pointer-events'], 'auto',
          'clicks not disabled when open is truthy');
  ...
  test('it passes and binds closeModal action to background and header (normal header)', async function(assert) {
    ...
    assert.equal(find('[data-test="rad-modal-background"]').getAttribute('aria-hidden'), 'false',
      'modal background state begins as visible');
    assert.equal(find('[data-test="rad-modal-wrapper"]').getAttribute('aria-hidden'), 'false',
    ...
    return settled().then(async () => {
      ...
      assert.equal(find('[data-test="rad-modal-background"]').getAttribute('aria-hidden'), 'true',
        'modal background state is !visible after close action fires from background click');
      assert.equal(find('[data-test="rad-modal-wrapper"]').getAttribute('aria-hidden'), 'true',
  ...
  test('it adds animation classes to the rad-modal-wrapper div', async function(assert) {
    ...
    assert.ok(find('.rad-modal-wrapper').classList.contains('animate-left'),
    ...
    assert.ok(find('.rad-modal-wrapper').classList.contains('animate-right'),
    ...
    assert.notOk(find('.rad-modal-wrapper').classList.contains('animate-'),
